...
public class Admindao {
	...
	public boolean addEvent(Events event) {
		boolean f=false;
		try {
			String query="insert into events(event_name, event_date, description, organizer, event_img) values(?,?,?,?,?)";
	...
	public boolean addAdmin(Admin admin) {
		...
		try {
			...
			ps.setString(4,admin.getProfile_img());
	...
	public Admin login(String email,String password) {
		...
		ResultSet rs=ps.executeQuery();
			while(rs.next()) {
				...
				admin.setProfile_img(rs.getString(4));
